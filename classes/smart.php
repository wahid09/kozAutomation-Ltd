...
//                    } else {
                        ...
                        extract($data);
                        $sql = "INSERT INTO tbl_smart (project_name, project_type, project_location, project_description, contact_person, image_one, image_two, image_three, image_four, image_five, video_link, publication_status)"
...
//            }
    ...
    public function select_all_project_info() {
        $sql = "SELECT * FROM tbl_smart WHERE deletion_status = 0";
        if (mysqli_query($this->link, $sql)) {
    ...
    public function select_project_info_by_id($project_id) {
        $sql = "SELECT * FROM tbl_smart WHERE project_id = '$project_id' ";
        if (mysqli_query($this->link, $sql)) {
            $query_result = mysqli_query($this->link, $sql);
            return $query_result;
        } else {
    ...
    public function unpublished_project_info_by_id($project_id) {
        $sql = "UPDATE tbl_smart SET publication_status = 0 WHERE project_id = '$project_id' ";
        if (mysqli_query($this->link, $sql)) {
    ...
    public function published_project_info_by_id($project_id) {
        $sql = "UPDATE tbl_smart SET publication_status = 1 WHERE project_id = '$project_id' ";
        if (mysqli_query($this->link, $sql)) {
...
//                        } else {
                            move_uploaded_file($_FILES['image_one']['tmp_name'], $target_file);
                            $sql = "UPDATE tbl_smart SET project_name = '$project_name', project_type = '$project_type', project_location = '$project_location', project_description = '$project_description', contact_person = '$contact_person', image_one='$target_file', video_link = '$video_link', publication_status = '$publication_status' WHERE project_id = '$project_id' ";
                            if (mysqli_query($this->link, $sql)) {
                                $_SESSION['message'] = "Project info update successfully";
                                header('Location: manage_project_smart.php');
                            } else {
                                die('Query problem' . mysqli_error($this->link));
                            }
                        }
...
//                }
            ...
            $sql = "UPDATE tbl_smart SET project_name = '$project_name', project_type = '$project_type', project_location = '$project_location', project_description = '$project_description', contact_person = '$contact_person', video_link = '$video_link', publication_status = '$publication_status'  WHERE project_id = '$project_id' ";
            if (mysqli_query($this->link, $sql)) {
                $_SESSION['message'] = "Project info update successfully";
                header('Location: manage_project_smart.php');
            } else {
...
//                        } else {
                            ...
                            move_uploaded_file($_FILES['image_four']['tmp_name'], $target_file_four);
                            move_uploaded_file($_FILES['image_five']['tmp_name'], $target_file_five);
                            $sql = "UPDATE tbl_smart SET image_one='$target_file', image_two='$target_file_two', image_three = '$target_file_three', image_four='$target_file_four', image_five='$target_file_five' WHERE project_id = '$project_id' ";
                            if (mysqli_query($this->link, $sql)) {
...
//                }
    ...
    public function delete_project_info_by_id($project_id) {
        $sql = "UPDATE tbl_smart SET deletion_status = 1 WHERE project_id = '$project_id' ";
//        $sql = "DELETE FROM tbl_smart WHERE project_id = '$project_id' ";
        if (mysqli_query($this->link, $sql)) {
